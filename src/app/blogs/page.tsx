...
export default function BlogsPage() {
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <h1 className="font-serif text-4xl text-teal-800 mb-6">Blog</h1>
      <ul className="space-y-6">
        {posts.map((post) => (
          <li key={post.slug} className="border-b border-black/10 pb-6">
            <h2 className="text-2xl font-serif text-teal-800">
              <Link href={`/blogs/${post.slug}`}>{post.title}</Link>
            </h2>
            <p className="text-sm text-black/70 mt-1">{new Date(post.date).toLocaleDateString()}</p>
            <p className="mt-2">{post.excerpt}</p>
            <Link className="mt-3 inline-block text-teal-700" href={`/blogs/${post.slug}`}>
              Read more →
            </Link>
          </li>
        ))}
